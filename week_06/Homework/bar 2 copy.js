...
d3.csv("Data/Races_Data.csv").then(data => {
    ...
    for (let d of data) {
        d.Population_Percentages = +d.Population_Percentages; //
    };
    ...
    let svg = d3.select("#another_chart") // D3 selection object encapsulates a set of DOM elements
    ...
    let y = d3.scaleLinear()
        .domain([0, d3.max(data, d => d.Population_Percentages)]).nice() // nice rounds the top num
    ...
    bar.append("rect") // add rect to bar group
        ...
        .attr("y", d => y(d.Population_Percentages)) // y position attribute
        .attr("height", d => y(0) - y(d.Population_Percentages)); // this height is the height attr on element
    
    bar.append("text") // add labels to the bars
        .text(d => d.Population_Percentages + '%')
        .attr("x", d => x(d.Race) + (x.bandwidth()/2))
        .attr("y", d => y(d.Population_Percentages) - 10)
