...
d3.json("libs/counties-albers-10m.json").then(us => {  
  // need = ["Arizona", "California"]
  us.objects.states.geometries = us.objects.states.geometries.filter(d => d.properties.name === 'Arizona'); // we are filtering to a proporty 
  // for 5 states, you make an array of those 5 states, and then use the same line
